    ...
    public function generateLink($id, $code) {
        try {
            ...
            if ($class->code == $code) {
                ...
                $shared_link = 'https://quizlet-client.herokuapp.com' . $owner->username . '/class?' . 'code=' . $code . '&id=' . $id;
                return response()->json([
                    "link" => $shared_link
                ], 200);
            }
...
//        else {
            ...
            $joinedClass = MembersHasClasses::where('member_id', '=', $user_id)
                ...
                ->first();
            if ($joinedClass != null) {
                return Redirect::to("https://quizlet-client.herokuapp.comoverview");
            }
            else {
                if ($realMember != null) {
                    $current_time = getCurrentTime();
            ...
            if ($user->id == $owner->id) {
                ...
                if ($class->public == 0) {
                    ...
                    if ($joinedMember != null) {
                        ...
                        $link = 'https://quizlet-server.herokuapp.com/api/class/join/confirm/' . $class_id . '/'. $user->id;
